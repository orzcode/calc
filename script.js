...
function operation(operator, symbol) {
  ...
  if (doubleOperator === true) {
    ...
    if (opInProgress === true) {
      ...
      console.log(
        "'Result' after doing a 'switched' (eg 2nd) operation: " + result
      );
      memoryValue = result;
...
function equals(type) {
  ...
  if (opInProgress === true && display.innerHTML === "0") {
    ...
    opInProgress = false;
    console.log(
      "After hitting Equals, result var is: " +
        result +
        " and type is " +
        typeof result
    );
...
function keyedNumber(keyedNumber) {
  ...
  console.log(
    "After pressing number, displayInnerHTML is " + display.innerHTML
  );
  console.log(
    "After pressing number, result is: " +
      result +
      " and type is " +
      typeof result
  );
...
function decimal() {
  switch (hasDecimal) {
    case false:
      ...
      console.log(
        "After pressing decimal, displayInnerHTML is " + display.innerHTML
      );
      break;
    case true:
      break;
